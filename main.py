...
# --- DATABASE CONFIG (SQLite instead of Postgres) ---
...
def upload():
    ...
    upload_result = cloudinary.uploader.upload(
        file,
        resource_type="raw",  # for non-image files like PDF, DOCX, etc.
    ...
    # Reset file stream for text extraction
    file.stream.seek(0)
    text_content = extract_text(file, file.filename)
...
@app.route('/initdb')
def initdb():
    db.drop_all()   # Drop old tables (reset)
    db.create_all() # Recreate tables
    return "✅ SQLite database initialized!"
